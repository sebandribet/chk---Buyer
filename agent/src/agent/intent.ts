...
function findGaps(raw: RawExtraction): ClarificationQuestion[] {
  ...
  if (raw.constraints.budget_ars === null && !todosConPlata) {
    gaps.push({
 ...
 */
function isBlockingQuestion(field: string, question: string): boolean {
...
export async function extractIntent(
  ...
  const modelQuestions: ClarificationQuestion[] = parsed.questions
    .filter((q) => isBlockingQuestion(q.field, q.question))
    .map((q) => ({
      field: q.field,
      question: q.question,
