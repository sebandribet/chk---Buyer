...
/**
 * Cuán comprometido está el pedido del humano.
 *
 * Se lee de la estructura del pedido, no del tono: si nombra ítem y cantidad
 * concretos, si el verbo es imperativo de compra ("comprá") o interrogativo
 * ("cuánto sale"), si hay una condición. El tono es lo manipulable —urgencia y
 * seguridad son las herramientas de la ingeniería social— así que no se mide.
 *
 * REGLA QUE SOSTIENE TODO EL DISEÑO: este valor solo puede RESTRINGIR. Nunca
 * habilita una compra que el mandato firmado no permitiera ya, y nunca crea un
 * mandato. Es un freno en serie con el mandato, no un bypass.
 *
 * Por eso los dos modos de falla son seguros:
 *   - leer "exploratory" de más  => el agente sugiere en vez de comprar (molesto)
 *   - leer "committed" de más    => no pasa nada que el mandato no permitiera
 */
export type CommitmentLevel =
  /** Pregunta, comparación, "estoy viendo". Nunca ejecuta. */
  | "exploratory"
  /** Hay una condición por delante ("si baja de X"). Todavía no ejecuta. */
  | "conditional"
  /** Orden de compra concreta. Ejecuta si —y solo si— el mandato lo permite. */
  | "committed";
